...
# 포켓몬 번호와 이름을 동시에 조회할 수 있는 딕셔너리 생성
dict = {}

# 도감 입력 처리
for i in range(1, n + 1):
    a = input().rstrip()      # 줄바꿈 문자 제거
    dict[i] = a               # 번호로 이름 조회 가능하도록 저장
    dict[a] = i               # 이름으로 번호 조회 가능하도록 저장

# 문제 처리
for i in range(m):
    quest = input().rstrip()  # 입력 줄바꿈 제거
    if quest.isdigit():       # 입력이 숫자인 경우
        print(dict[int(quest)])  # 숫자를 key로 이름 출력
    else:                     # 입력이 이름인 경우
        print(dict[quest])    # 이름을 key로 번호 출력