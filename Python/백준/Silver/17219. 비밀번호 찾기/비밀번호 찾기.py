...
n, m = map(int, input().split())  # 저장된 사이트 개수 N, 찾으려는 사이트 개수 M

site_dict = {}  # 사이트 주소와 비밀번호를 저장할 딕셔너리

# N개의 사이트 주소와 비밀번호 입력받아 저장
for _ in range(n):
    site, pwd = input().split()  # 사이트 주소, 비밀번호 분리
    site_dict[site] = pwd        # 딕셔너리에 {사이트: 비밀번호} 형태로 저장

# M개의 사이트 주소를 입력받아 비밀번호 출력
for _ in range(m):
    search = input().strip()   # 찾으려는 사이트 주소
    print(site_dict[search])   # 딕셔너리에서 비밀번호 꺼내서 출력
